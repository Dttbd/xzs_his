...
// 工厂：real 用 resolver、忽略 sink；mock 用 sink、忽略 resolver
func New(cfg config.WeChatConfig, resolver UserResolver, sink MockSink) Client {
    if cfg.Enabled {
        return newRealClient(cfg, resolver)
    }
...
构造 `wechat.New(cfg.WeChat, userResolver, mockSink)`。`UserResolver` 由 user repository 实现，`MockSink` 由 notification service 实现（薄适配，复用 `SendNotification`，type 传 `wechat_mock`）。
...
- `cmd/server/main.go`、`cmd/worker/main.go`（装配 wechat.New + 条件注册 dev-login）
- 前端管理端：登录页按钮 + 回调接收页 + shared api

## 10. 非目标（YAGNI）

- 不实现客户门户的企微登录（客户用账号密码）
- 不做 Redis 共享 token 缓存（进程内足够）
- 不做企微通讯录同步 / 部门导入
- 不做除 textcard 外的消息类型
